...
'405 - White River Juncti'=>{'fyCol'=>[2, 0, 0, 0, 0, 0], 'ttlPending'=>36, 'bfDocDate'=>2}}")
  ...
  def getAnalysis
  	...
  	if (@@docketReport.nil?)
  		flash[:notice] = "Please create report first."
  	else
  		@temp = Vacols::Analyze.new(@numJudge, @judgeMult, @coDays, @@docketReport, @@ttlbfDocDate)
  		flash[:notice] = @temp.allocate_video_hearings
  	end
